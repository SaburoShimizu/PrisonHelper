...
function imgui.OnDrawFrame()
    ...
    if prisonmenu.v then
        ...
        imgui.Begin(u8'Настройки PrisonHelper v'..thisScript().version, prisonmenu, imgui.WindowFlags.NoSavedSettings + imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.MenuBar)
		if imgui.BeginMenuBar() then
			...
			if imgui.BeginMenu(u8'FAQ') then
				if imgui.MenuItem(u8'Показать FAQ') then
					prisonmenu.v = false
					prishelp.v = true
				end
				if imgui.MenuItem(u8'Показать график') then
					prisonmenu.v = false
					grafeks.v = true
				end
				imgui.EndMenu()
    ...
    if pris.fastmenu then
        if fastmenus.v then
            ...
            if imgui.CollapsingHeader(u8'Быстрый репорт (Своя причина)') then
				...
				if imgui.MenuItem(u8'Отправить жалобу (x2 Enter)') then if #jbsuka.v >1 and jbsuka.v ~= '' and jbsuka.v ~= ' ' and jbsuka.v ~= '  ' then SendReportDialog(id, name, u8:decode(jbsuka.v)) jbsuka.v = nil fastmenus.v = false else sampAddChatMessage(teg..'Вы ничего не ввели', -1) end end
    ...
    if imguilec.v then
        ...
        imgui.Begin(u8'Лекции', imguilec, imgui.WindowFlags.NoSavedSettings + imgui.WindowFlags.AlwaysAutoResize)
